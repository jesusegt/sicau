<?php 
	class Asistencia {
		...
		/* REGISTRAR */
			public function RegistrarAsistencia($cedula, $fecha, $hora, $hora_salida){
				require_once("conexionpdo.php"); //se llama al archivo para la conexion
				...
				if(empty($data)){ //Si el método, retorna un arreglo vacío;
					echo "<script>alert('Obrero no existente, si usted es un Obrero de la institución contacte al encargado del sistema para ser registradoa.')</script>";//Mensaje de Registro no válida
										...
										foreach($data as $a){
											...
											if(empty($data)){
												...
												if($accion == 'entrada'){
													...
													$sql = "INSERT INTO asistencia (fecha, hora, accion, id_sol)VALUES (:fecha, :hora_salida, :accion, :idsol)"; //sentencia sql para registrar 
													$insert = $con->prepare($sql); //preparar la sentencia sql
													//Excecute
													$insert->execute(array('fecha'=>$fecha, 'hora_salida'=>'20:33:15', 'accion'=>'Salida', 'idsol'=>$idsol));
